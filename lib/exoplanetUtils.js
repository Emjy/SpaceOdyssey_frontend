...
export function parseExoplanetSystems(rows) {
    ...
    return systems.map(({ star, planets }) => {
        ...
        // Zone habitable — st_lum est en log10(L/L☉) dans la NASA archive
        const hzLum = star.st_lum != null
            ? Math.pow(10, star.st_lum)
            : Math.pow(star.st_rad ?? 1, 2) * Math.pow((star.st_teff ?? 5778) / 5778, 4);
